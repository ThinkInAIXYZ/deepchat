...
3. 继承父 `projectDir`
4. 继承父 session model / permission / generation settings
5. child session 写入 `session_kind='subagent'`
6. child session 写入 `parent_session_id=父 sessionId`
7. child session 写入 `subagent_meta_json`
