...
- As a DeepChat desktop user, I want Discord remote control settings to stay separate from Discord webhook notifications, so changing one does not mutate the other.
...
- Discord remote control does not read or mutate `hooksNotifications.discord`.
...
- The renderer shows a dedicated Discord tab in remote settings, and that tab does not expose Discord webhook notification fields.
...
- No Discord notification webhook migration.
