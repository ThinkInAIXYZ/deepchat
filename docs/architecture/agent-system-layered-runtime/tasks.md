...
> 状态：architecture ready；implementation not started。  
...
- [ ] `ASLR-003` Add missing characterization tests and machine-readable architecture baseline.
...
- [ ] `ASLR-055` Move tool catalog/execution/result normalization adapters.  
  Depends on: `ASLR-054`.
- [ ] `ASLR-056` Move pre-check, question, post-call permission and skill-draft outcomes into an ordered batch; intermediate responses stay paused.  
...
- [ ] `ASLR-061` Add `MemoryIngestionObserver` with extraction/projection/cursor parity.  
