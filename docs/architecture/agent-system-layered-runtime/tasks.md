...
> 状态：implementation in progress；ASLR-000..034、ASLR-040..046、ASLR-050..057、ASLR-070..073 completed。
...
- [x] `ASLR-051` Extract current provider round/tool loop into `DeepChatLoopEngine`.
  Depends on: `ASLR-050`.
...
- [x] `ASLR-055` Move tool catalog/execution/result normalization adapters.
  Depends on: `ASLR-054`.
- [x] `ASLR-056` Move pre-check, question, post-call permission and skill-draft outcomes into an
...
- [x] `ASLR-057` Adapt external hook notifications as non-blocking observers.
  Depends on: `ASLR-052`.
...
- [x] `ASLR-072` Switch only `kind=acp` routing with message/Tape/event/trace parity, retaining DeepChat + ACP-provider compatibility.
  Depends on: `ASLR-071`.
- [x] `ASLR-073` Remove migrated ACP runtime methods from generic LLM provider contracts.
  Depends on: `ASLR-072`.
