...
| Tape facts, effective view, manifest, replay/privacy | existing Tape fact/view/replay suites | recorder/output causal order in `ASLR-052`; pure-read causal join, partial/unavailable states and event-history gap in `ASLR-080`; full non-interference in `ASLR-081` |
