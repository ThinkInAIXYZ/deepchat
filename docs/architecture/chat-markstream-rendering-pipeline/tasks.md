...
- [ ] 让正常尾部 inline stream 命中 stable/tail layout contract，并覆盖中间 stream 回退。
- [ ] 修正高 orderSeq 分页窗口中的 optimistic/stream 本地尾序。
- [ ] 使用 shallow stream state，并预填 renderer parsed cache；保留 main JSON normalization 兼容语义。
...
- [ ] 在 Electron/Chromium 中手工验证完成态 `stream-diffs` gutter/content 几何；jsdom 不具备该布局能力。
