...
- [x] 把 Window、Tab、Shortcut 和 YoBrowser 的共享合同移到 `types/desktop.ts`；删除旧
...
- [x] 把每个通过的设计阶段拆成可单独检查的实施批次。
- [x] 每次移动职责前先检查已有测试；只有关键行为没有覆盖时才增加最小测试。
- [x] 每批只迁移一组调用方。
- [x] 同一批删除对应旧依赖路径。
- [x] 同一批移动对应实体文件，不积累到最后统一搬迁。
- [x] 每个目标边界落地后增加自动依赖检查。
- [x] 每批更新描述当前实现的架构和流程文档。
- [x] 保持现有数据和有类型的外部通信约定。
- [x] 确认没有为迁移保留临时兼容代码。
- [x] 每批完成与影响范围相符的局部和完整验证。
- [x] 重新生成最终依赖基线，并确认结果符合目标依赖图。

## T9：完成检查

- [x] 确认每份可变状态和运行资源都有唯一负责模块。
- [x] 确认全局 `Presenter`、全局模块查找入口和替代它的新总入口都已删除。
- [x] 确认固定启动和退出顺序已经实现，清理可以重复调用。
- [x] 确认 Desktop、Remote、Scheduler、deeplink 和 subagent 共用 Session 规则。
- [x] 确认 `DeepChat` 和 `ACP` 仍是分开的有类型运行实现。
- [x] 确认 Config、SQLite、route 和 event 不再充当业务总入口。
- [x] 删除全局 `EventBus`、`sendToMain()` 和对应测试；Provider DB 使用模块内有类型通知，
  MCP 和其他业务操作使用直接调用。
- [x] 确认用户数据、route/event 和用户可见行为保持兼容。
- [x] 把当前架构文档更新到最终实际实现。
- [x] 运行最终 format、i18n、lint、typecheck、main/renderer test 和相关 E2E。
