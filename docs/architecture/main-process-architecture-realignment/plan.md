...
> 实施限制：Session 部分仍有 `[NEEDS CLARIFICATION]` 或尚未通过确认时，不能修改生产架构。
...
只有已经确认的结论才写入 [spec.md](./spec.md) 的“已确定的结论”表。尚未确定的内容继续标记
`[NEEDS CLARIFICATION]`。
...
1. 先用测试固定即将移动的现有行为；
2. 增加已确认的负责模块，或把状态移给它，同时保持 route 约定；
3. 只迁移一组调用方；
4. 同一批删除这组调用方的旧依赖路径；
5. 增加自动依赖检查；
6. 更新描述当前实现的文档；
...
每一批改动都应能在不降级用户数据的情况下回退。非必要不改 schema。确实需要临时兼容代码时，
必须满足：
