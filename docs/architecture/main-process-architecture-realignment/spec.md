...
| 已无实际作用 | `SHORTCUT_EVENTS.ZOOM_IN/OUT/RESUME` 的 main event、没有 main 接收方的 window 状态 event、`LIFECYCLE_EVENTS.SHUTDOWN_REQUESTED`、`providerUpdated` | 删除 main `EventBus` 调用；已经存在的 typed renderer event 保持不变。 |
