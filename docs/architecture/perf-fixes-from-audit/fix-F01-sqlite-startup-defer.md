# 修复 F1：SQLite 启动关键路径收缩与后台诊断重排

## 目标
缩短 DeepChat 主进程启动关键路径中的 SQLite 阻塞时间，但不改变当前数据库初始化的正确性边界：
- 关键路径仍保留数据库打开、最小连接验证、建表、版本表初始化、迁移与必要的启动期 schema 处理。
- 明确把可异步执行的 schema 诊断/修复建议放到持有 `StartupWorkloadCoordinator` 的层调度，而不是假设 `SQLitePresenter` 自己能直接调度 coordinator。
...
## 修复方案
1. **后台调度放在持有 coordinator 的层**
   - 不在文档中假设 `SQLitePresenter` 直接依赖 `StartupWorkloadCoordinator`。
   - 可行接入点应为：
     - `lifecyclePresenter`：它已持有 `startupWorkloadCoordinator`，可在创建/管理初始化流程时把 `runId` 与 coordinator 传给数据库初始化层。
     - 或 `DatabaseInitializer`：通过显式依赖注入接收 `startupWorkloadCoordinator` 与启动上下文（至少包含 `target`、可选 `runId`）。
   - 推荐形式是给 `DatabaseInitializer` 增加可选构造参数，例如 `{ startupWorkloadCoordinator, startupTarget: 'main', startupRunId }`，再由它在 `initialize()` 完成关键路径后注册后台诊断任务。
   - 这样可以保持 `SQLitePresenter` 只负责数据库能力，后台任务编排仍归属于生命周期/启动编排层。
...
3. **关键路径与后台任务重新分界**
   ...
   - 后移到 background：
     - 仅用于观察/告警的 schema 诊断
     - `manualIssues` 告警整理
     - 若后续增加更温和的补充检查，也应通过 background task 执行
...
5. **后台诊断任务的接入方式**
   - 在 `DatabaseInitializer.initialize()` 成功返回数据库实例后，由持有 coordinator 的层注册后台任务，而不是在 `SQLitePresenter` 构造函数里 fire-and-forget。
   - 任务建议参数：
     - `id`: 例如 `main:sqlite-schema-diagnosis`
     - `target`: `'main'`
     - `phase`: `'background'`
     - `resource`: `'io'`
     - `labelKey`: 新增数据库诊断对应的 i18n key
     - `runId`: 复用当前主启动 runId（如已传入）
   - 任务体内部再调用 `database.diagnoseSchema(catalog)` 或 `DatabaseInitializer` 暴露的诊断辅助方法，并记录 diagnose duration、输出 repairable/manual issue 摘要。
...
2. **再引入 coordinator 依赖注入**
   - 从持有 `startupWorkloadCoordinator` 的上层，把 coordinator、`target: 'main'`、`runId` 显式传给 `DatabaseInitializer`。
   - `DatabaseInitializer` 只在这些依赖存在时才注册后台任务，避免影响非启动场景或测试构造。

3. **最后把纯观察性诊断迁移到 background**
   - 先确认同步 diagnose 的职责里哪些是“启动正确性必需”，哪些只是“告警增强”。
   - 只有当同步 repair 判定链路有明确替代方案后，才把对应诊断从关键路径移走。
...
- 代码落地后应验证：
  ...
  - 主启动链路若注册后台任务，实际使用 `target: 'main'` 且 runId 来自当前主启动 run。
...
- **后台任务依赖 run 上下文**：如果 `DatabaseInitializer` 在测试、CLI 或其他非主启动路径复用，注入的 coordinator / runId / target 可能不存在，必须保持依赖可选且行为可降级。
