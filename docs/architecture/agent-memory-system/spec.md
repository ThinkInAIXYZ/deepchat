...
| Tools | `tool/agentTools/agentMemoryTools.ts` | `memory_remember` / `memory_recall` / `memory_forget` |
| Tools | `tool/agentTools/agentTapeTools.ts` | 模型只使用 `tape_search` / `tape_context`；info、anchors 和 handoff 不对模型开放 |
