...
- **Silent FTS-only forever** if a bundled extension fails to `LOAD` by path: mitigate by logging on
  fallback and covering load-by-path in the smoke check.
...
- Build/smoke — `smoke:duckdb:vss` loads the bundled extension by path on macOS/win/linux without
  network install.
