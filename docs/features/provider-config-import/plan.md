...
- Add welcome guide import action beside the Select a Provider step title; it opens settings route `settings-database` with section `provider-import` and resumes the onboarding guide on the next provider setup step without completing setup until import succeeds.
