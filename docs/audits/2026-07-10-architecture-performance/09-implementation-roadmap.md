...
| `DCL-002` | 已本地合入 | local fast-forward `12e2df1f` | node/web 各自用真实 compiler options 检查全部 19 个 shared declarations，均为 `0 diagnostics`；strict diagnostic、partial-root、zero-root targeted 反例 final `4/4`，证明 `skipLibCheck`、tsconfig include/exclude 和空 root 都不能让 gate 静默通过；完整 typecheck、format、i18n、lint、architecture guard、diff check 通过；独立审查先发现 partial-root fail-open，修复后 `PASS / 0 BLOCKER`；未跑 full/E2E | 新 `typecheck:declarations` 已串到现有 `typecheck → build → PR check` 链，不增加重复 CI job。文件系统枚举保证 shared declaration 全量，node/web config 只提供各自真实 resolution options；shared→main alias 仍由 architecture lint gate 负责。范围有意只覆盖 repo-owned `src/shared/**/*.d.ts`，不把全部第三方 declaration 或 Vue SFC 重做一遍 |
| `HIS-001` | 已本地合入 | local squash `fff26807` | clean Electron ABI143 + real SQLite 跑完 `10/100/1k/10k messages × 0/10k/100k global traces`，每组 `1 warmup + 5 measured`，raw 为 12×5 且 deterministic report byte-exact；每次 send 固定 `8` 次完整 history read，真实 SQL 为 `120 / 840 / 8,040 / 80,040`；12 个 query plan 都出现 global trace materialize/scan。renderer report tests `2/2`、quick native `1/1`、`typecheck:node`、format、i18n、lint、diff check 通过；独立审查两次阻止把 provider 边界外 timer 计入结果，最终 `PASS / 0 BLOCKER`；production `src/**` 净差异为 `0`，未跑 full/E2E | 证据同时确认原 P-01/P-02，并发现空 file/link bucket 会按每个 user message 再走 fallback query 的真实 N+1；10k fixture 在本机 provider-start median 约 `0.93–0.97s`，只作为 Apple M5 本机基线，不外推。60/60 event-loop probe 均在 provider 前未触发，按 `null/censored` 保存，不声称 CPU/event-loop 收益。`HIS-002/003` 解锁，`HIS-004` 仍须等 predicate/projection contract 稳定；本切片只交付 test/script fixture 和证据，没有 runtime hook、flag、事件总线、schema/query/cache 改动。完成态 `plan.md/tasks.md` 已清理，只保留 maintained spec 与 results |
| `HIS-002` | 已本地合入 | local squash `554a4b7b` | required `hasMessages()` 从 real SQLite table 贯通 MessageStore、AgentRuntime 到 AgentSession；main focused `337 passed / 2 ABI skipped`，Electron ABI143 real SQLite existence/query-plan `3/3`；typecheck、format、i18n、lint、diff check 通过；HIS-001 direct quick 从 `8 → 7` 次完整 read、`120 → 105` 条 history SQL，正式 raw/report hash 前后一致；独立审查 `PASS / 0 BLOCKER`，未跑 full/E2E | `sendMessage()` 不再为一个 boolean 读取 message IDs、content、structured rows 或 trace count，empty/non-empty/draft/title 与错误传播语义不变；boolean-only transfer/reusable-draft helper 复用同一 required API，但保留既有 conservative-true catch。没有 optional fallback、runtime flag、schema/query migration、UI 或 HIS-003/004 改动。empty file/link N+1 与 global trace projection 仍明确留给后续独立切片；完成态 `plan.md/tasks.md` 已清理，只保留 maintained spec |
| `HIS-003A` | 已本地合入 | local squash `6d24d724` | MessageStore + report focused `36/36`；Electron ABI143 direct quick `1/1`，在 HIS-002 的 `7 reads / 105 SQL` 上进一步收敛为 `7 / 35`，header/structured rows 均为 `70`；正式 raw/report byte-exact 且 SHA-256 前后一致；typecheck、format、i18n、lint、architecture/process guards、diff check 通过；独立审查 `PASS / 0 BLOCKER`，未跑 full/E2E | bulk structured maps 已经知道某 message 没有 file/link/assistant rows 时，该 empty bucket 现在是 authoritative，不再逐 user message重新查库；legacy missing user structured row 仍回退 header content，assistant empty structured rows仍回退 raw content，正确性兼容不变。benchmark exact guard 从单 read `15` 更新为 `5`，renderer 按 raw shape 区分历史 N+1 与新无 per-message fallback，不篡改 HIS-001 历史结果。未改 trace query/projection、schema、cache、snapshot 或 UI；此局部切片无需新增完成态 SDD 实体 |
| `HIS-003B` | 已本地合入 | local squash `99d7ac6d` | Electron focused `340/340`，另有 Tape focused `53/53`；real SQLite runtime plan 不含 trace table 或 `MATERIALIZE`，rich projection仍返回真实 `traceCount=2`；direct quick 精确为 `7 total / 0 rich / 7 runtime reads`、`0 rich / 7 runtime headers`、`35 SQL`；legacy Tape非零 `traceCount=7` 可读，新 backfill全部为0；typecheck、format、i18n、lint、architecture/process guards、diff check通过，正式raw/report hash不变；独立审查 `PASS / 0 BLOCKER`，未跑full/E2E | public rich history、paged UI、single-message和mutation paths保持trace metadata contract；pending/question guard、context/compaction fallback与Tape full backfill改走trace-free projection。Trace UI/replay trace payload仍以 `deepchat_message_traces` 为真源，历史Tape不迁移、不重写；新 backfill中的 `traceCount=0` 是明确compatibility metadata。没有第二套message domain model、runtime flag、schema、cache、watermark、snapshot或UI改动；完成态plan/tasks已清理，只保留projection与Tape maintained spec |
| `HIS-004` | 已本地实施，待合入 | docs `35ec191c` / implementation `6d694459` | Electron ABI143 AgentRuntime/Tape focused `241/241`；real SQLite direct quick 精确为 `2 total / 0 rich / 2 runtime reads`、`0 rich / 2 runtime headers`、`10 SQL`，query plan不含trace table或`MATERIALIZE`；deferred hydration mutation、pending block、process/Tape零重读、direct/retry fresh、pre-stream clear/cancel、completed drain fresh identity、rapid steer与Tape copy-sort反例通过；`typecheck:node`、format、i18n、lint、architecture/agent-cleanup/process guards、diff check通过，formal raw/report hash不变，独立审查 `PASS / 0 BLOCKER`，未跑full/E2E/build | 有意保留AgentSession外层state与AgentRuntime turn两个owner-local trace-free snapshots，不为追求1 read引入跨owner token/cache/invalidation。runtime snapshot只在settings hydration后局部传递给queue guards、initial process guard与Tape backfill；无map/TTL/revision/flag/eventbus/public route/interface/第二message model/schema/UI/watermark。精确2/10只在10-message/0-trace quick复验，不覆盖历史12-scenario baseline；完成态plan/tasks已清理，保留maintained contract |
| `SDD-CLEANUP` | 已本地合入 | local fast-forward `97e45164` | 两个独立只读 reviewer 按代码、提交历史、checkbox、GitHub issue和引用图复核；删除18个已完成/过时的交付文件并更新6个retained specs，共净减556行；17个仍有unchecked task的目录、Scheduler/OAuth manual-pending、database results与performance report均保持不动；deleted path与相对链接引用扫描为0，diff check和clean status通过，最终 `PASS / 0 BLOCKER` | 只清理已完成目标的plan/tasks和一个已由代码+测试证明修复的issue目录；保留维护contract、机器基线、raw/report及仍被引用的历史报告。`perf-fixes-from-audit`虽checkbox全勾选，但F11是“尝试后未达验收”的关闭决策且tasks仍被对账文档引用，因此明确保留；未运行测试/E2E，因为本切片只删过程文档且未改代码 |
...
| 20 | `HIS-004` | P-01 | L | `HIS-002`, `HIS-003` | 保留两个owner-local runtime snapshots，复用AgentRuntime turn内的单次读取 | real SQLite direct quick精确 `2 reads / 10 SQL`；pending/question、hydration freshness、cancel与completed-drain正确性不回退 |
...
| P-01/P-02 | 一次 send 重复完整物化 history，rich read 又全局聚合 trace | 先埋点，再分 existence/predicate 和 UI rich view，最后复用两个owner-local snapshots | 长会话发送前等待更稳定，trace 很多时不拖累正常对话 | direct quick收敛为2 reads/10 SQL；拒绝为1 read增加跨owner cache/token/invalidation状态 | `HIS-001`–`HIS-004` |
