...
export class UserPlugins {
  ...
  install(input: UserPluginInstallInput): Promise<PluginActionResult> {
    return this.result(async () => {
      if (this.deps.store.pending())
        throw new Error(
          'Restart MioWork to recover the interrupted plugin update before applying another change'
        )
  ...
  enable(pluginId: string): Promise<PluginActionResult> {
    return this.result(async () => {
      if (this.deps.store.pending())
        throw new Error(
          'Restart MioWork to recover the interrupted plugin update before enabling plugins'
        )
  ...
  configureMcp(
    ...
    return this.result(async () => {
      if (this.deps.store.pending())
        throw new Error(
          'Restart MioWork to recover the interrupted plugin update before changing MCP configuration'
        )
      const record = this.record(pluginId)
