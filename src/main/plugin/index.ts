...
export class PluginService implements PluginServicePort {
  ...
  private createCuaRuntimeIntegrityVerifier(
    ...
    if (plugin.integrityError) {
      throw new Error(
        `CUA runtime integrity descriptor is unavailable: ${plugin.integrityError}. Reinstall MioWork or the CUA plugin.`
      )
    }
    ...
    if (!descriptor || !runtime?.adapterContract) {
      throw new Error(
        'CUA runtime integrity descriptor is missing. Reinstall MioWork or the CUA plugin.'
      )
    }
  ...
  private async refreshRuntime(pluginId: string): Promise<PluginRuntimeStatus> {
    ...
    if (pluginId === CUA_PLUGIN_ID && runtimeManifest.adapter !== 'cua-embedded-v1') {
      throw new Error(
        'The installed CUA runtime uses an unsupported legacy lifecycle. Repair or reinstall MioWork before enabling Computer Use.'
      )
    }
