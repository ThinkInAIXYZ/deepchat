...
export class VertexProvider extends BaseLLMProvider {
  ...
  private resolveProjectId(): string | undefined {
    return this.vertexProvider.projectId || process.env.GOOGLE_CLOUD_PROJECT
  }

  private resolveLocation(): string | undefined {
    return this.vertexProvider.location || process.env.GOOGLE_CLOUD_LOCATION
  }

  private buildGoogleAuthOptions() {
    const privateKey = this.vertexProvider.accountPrivateKey
    const clientEmail = this.vertexProvider.accountClientEmail
  ...
  private buildBaseUrl(): string {
    const customBaseUrl = this.vertexProvider.baseUrl?.trim()
    if (customBaseUrl) return customBaseUrl

    const location = this.resolveLocation() || 'us-central1'
    if (this.vertexProvider.endpointMode === 'express') {
      return 'https://aiplatform.googleapis.com/'
    }
    return `https://${location}-aiplatform.googleapis.com/`
  }

  private getApiVersion(): 'v1' | 'v1beta1' {
    return (this.vertexProvider.apiVersion as 'v1' | 'v1beta1') || 'v1'
  }

  private createGenAIClient(): GoogleGenAI {
    ...
    return new GoogleGenAI({
      ...
      location,
      apiVersion,
      googleAuthOptions: this.buildGoogleAuthOptions(),
      httpOptions: {
  ...
  // 确保带有 Vertex 格式的模型路径
  private ensureVertexModelName(modelId: string): string {
  ...
  protected async fetchProviderModels(): Promise<MODEL_META[]> {
    try {
      ...
      if (models.length === 0) {
        console.warn('No models found in Vertex AI response, using Provider DB models')
      ...
      // console.log('Mapped Vertex models:', apiModels)
      return apiModels
    } catch (error) {
      console.warn('Failed to fetch models from Vertex AI:', error)
  ...
  public async summaryTitles(
    ...
    try {
      ...
      const result = await this.genAI.models.generateContent({
        model: this.ensureVertexModelName(modelId),
  ...
  // Override fetchModels method since Vertex AI will reuse the cached model list
  async fetchModels(): Promise<MODEL_META[]> {
    // Vertex AI will reuse the cached model list
    return this.models
  }
  ...
  async check(): Promise<{ isOk: boolean; errorMsg: string | null }> {
    try {
      ...
      if (!projectId || !location) {
        return { isOk: false, errorMsg: 'projectId and location are required for Vertex AI' }
      }
      if (this.vertexProvider.accountPrivateKey && !this.vertexProvider.accountClientEmail) {
        return {
          isOk: false,
          errorMsg: 'accountClientEmail is required when accountPrivateKey is provided'
        }
      }
      this.genAI = this.createGenAIClient()
      ...
      const result = await this.genAI.models.generateContent({
        model: this.ensureVertexModelName(testModelId),
        contents: [{ role: 'user', parts: [{ text: 'Hello from Vertex AI' }] }]
      })
      return {
        isOk: Boolean(result && (result.text || result.candidates?.length)),
        errorMsg: null
      }
    } catch (error) {
      console.error('Vertex provider check failed:', this.provider.name, error)
  ...
  protected async init() {
    if (this.provider.enable) {
      try {
        ...
        const location = this.resolveLocation()
        if (!projectId || !location) {
          console.warn('Vertex provider missing projectId or location, skip initialization')
          return
        }
        this.genAI = this.createGenAIClient()
        ...
        // Vertex AI is relatively slow, special compensation
  ...
  private formatVertexMessages(messages: ChatMessage[]): {
  ...
  // 处理 Vertex API 响应，支持新旧格式的思考内容
  private processVertexResponse(result: any): LLMResponse {
  ...
  async completions(
    ...
    try {
      ...
      const { systemInstruction, contents } = this.formatVertexMessages(messages)
      ...
      try {
        ...
        console.warn('Failed to estimate token count for Vertex response', e)
      }
      ...
      console.error('Vertex completions error:', error)
      throw error
    }
  }
  ...
  async summaries(
    ...
    try {
      ...
      return this.processVertexResponse(result)
    } catch (error) {
      console.error('Vertex summaries error:', error)
      throw error
    }
  }
  ...
  async generateText(
    ...
    try {
      ...
      console.error('Vertex generateText error:', error)
      throw error
    }
  }
  ...
  async suggestions(
    ...
    try {
      ...
      try {
        ...
        console.error('Vertex suggestions parseError:', parseError)
        // 如果解析失败，尝试分行处理
      ...
      console.error('Vertex suggestions error:', error)
  ...
  private async *handleImageGenerationStream(
    ...
    try {
      ...
      // 构建包含文本和图片的parts数组，参考formatVertexMessages的逻辑
      const parts: Part[] = []
  ...
  async getEmbeddings(modelId: string, texts: string[]): Promise<number[][]> {
    ...
    // 若无返回，抛出异常
    throw new Error('Vertex AI embedding API did not return embeddings')
  }
}
