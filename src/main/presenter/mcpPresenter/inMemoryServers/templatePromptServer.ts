...
// --- MCP Server 实现 ---
export class TemplatePromptServer {
  ...
  private listTools() {
    return {
      tools: [
        {
          name: 'list_all_template_names',
          description: '获取所有可用模板的名称列表。',
          inputSchema: zodToJsonSchema(z.object({})) // 无需参数
        },
        {
          name: 'get_template_parameters',
          description: '根据模板名称获取其所需的参数列表和描述。',
          // inputSchema 现在是根据 TemplateParameterSchema 生成的，不包含 type
          inputSchema: GetTemplateParametersArgsJsonSchema
        },
        {
          name: 'fill_template',
          description: '根据模板名称和参数，填充模板内容并生成最终的Prompt。',
  ...
  private async handleToolCall(request: z.infer<typeof CallToolRequestSchema>) {
    ...
    if (name === 'list_all_template_names') {
      // 1. 得到所有模板名
      try {
        const templates: TemplateDefinition[] = await presenter.configPresenter.getCustomPrompts()
        ...
        throw new Error('无法获取模板名称列表。')
      }
    } else if (name === 'get_template_parameters') {
      // 2. 得到模型的参数等信息
      const parsed = GetTemplateParametersArgsSchema.safeParse(args)
      if (!parsed.success) {
        throw new Error(
          `Invalid parameters for get_template_parameters: ${parsed.error.errors.map((e) => e.message).join(', ')}`
        )
      }
    ...
    } else if (name === 'fill_template') {
      // 3. 填充模板，得到prompt
      const parsed = FillTemplateArgsSchema.safeParse(args)
      if (!parsed.success) {
        throw new Error(
          `Invalid parameters for fill_template: ${parsed.error.errors.map((e) => e.message).join(', ')}`
        )
      }
