...
export class AgentLoopHandler {
    ...
    this.toolCallProcessor = new ToolCallProcessor({
      getAllToolDefinitions: async (context) => {
        ...
        if (context.conversationId) {
          try {
            const session = await presenter.newAgentPresenter.getSession(context.conversationId)
  ...
  private async resolveWorkspaceContext(
    ...
    if (!conversationId) {
      const rawFallback = this.options.configPresenter.getSetting<'agent' | 'acp agent'>(
        'input_chatMode'
      )
      return {
        chatMode: rawFallback === 'acp agent' ? 'acp agent' : 'agent',
    ...
    try {
      ...
      const resolvedModelId = modelId?.trim() || session.modelId?.trim() || ''
      if (
        session.providerId === 'acp' ||
        (resolvedModelId && session.agentId === resolvedModelId)
      ) {
        return {
          chatMode: 'acp agent',
          agentWorkspacePath: session.projectDir ?? null
        }
      }
      ...
      console.warn(
        '[AgentLoopHandler] Failed to resolve workspace context from new session:',
        error
      )
      const rawFallback = this.options.configPresenter.getSetting<'agent' | 'acp agent'>(
        'input_chatMode'
