...
const AGENTPOND_TRACING_STATE = Symbol.for('deepchat.agentpond-tracing')
  ...
  try {
    ...
    return {
      ...
      telemetry: {
        ...
        integrations: [
          new OpenTelemetry({
            tracer: provider.getTracer('deepchat'),
            usage: true
          }),
          {
    ...
    logger.warn('[AgentPond] Failed to initialize tracing', error)
    return undefined
  }
}
...
export async function flushAgentPondTracing(): Promise<void> {
  try {
    ...
    logger.warn('[AgentPond] Failed to flush tracing', error)
  }
}
