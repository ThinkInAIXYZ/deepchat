/**
 * Pure rules for routing window-level typing and paste into the chat composer.
 * Callers supply event ownership so these rules do not depend on DOM state.
 */

export type ComposerKeydownIntent =
...
export type ComposerInputRoutingContext = {
...
export function shouldRouteComposerPaste(
  event: Pick<ClipboardEvent, 'defaultPrevented'>,
  context: ComposerInputRoutingContext
): boolean {
  return (
    !event.defaultPrevented &&
    context.isEnabled &&
    !context.isEditableTarget &&
    !context.hasInteractiveFocus
  )
}
...
export function resolveComposerKeydownIntent(
  event: KeyboardEvent,
  context: ComposerInputRoutingContext
