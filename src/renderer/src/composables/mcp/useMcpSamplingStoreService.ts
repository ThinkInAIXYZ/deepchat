...
import { useMcpSamplingAdapter } from '@/composables/mcp/useMcpSamplingAdapter'
import type {
  McpSamplingDecision,
...
export const useMcpSamplingStoreService = () => {
  const mcpAdapter = useMcpSamplingAdapter()
  ...
  const submitDecision = async (decision: McpSamplingDecision) => {
    ...
    try {
      await mcpAdapter.submitSamplingDecision(decision)
      clearRequest()
    } catch (error) {
      console.error('[MCP Sampling] Failed to submit decision:', error)

      try {
        await mcpAdapter.cancelSamplingRequest(
          activeRequestId,
