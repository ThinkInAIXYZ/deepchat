...
import { useModelAdapter } from '@/composables/model/useModelAdapter'
...
export const useModelStoreService = () => {
  const modelAdapter = useModelAdapter()
  ...
  const isAgentProvider = async (providerId: string): Promise<boolean> => {
    try {
      return Boolean(await modelAdapter.isAgentProvider(providerId))
    } catch (error) {
  ...
  const getProviderModelsQuery = (providerId: string) => {
    return ensureQueryHandle(providerModelQueries, providerId, {
      ...
      staleTime: 30_000,
      query: async () => modelAdapter.getProviderModels(providerId)
    })
  }
  ...
  const getCustomModelsQuery = (providerId: string) => {
    return ensureQueryHandle(customModelQueries, providerId, {
      ...
      staleTime: 30_000,
      query: async () => modelAdapter.getCustomModels(providerId)
    })
  }
    ...
    return ensureQueryHandle(enabledModelQueries, providerId, {
      ...
      query: async () => {
        const [providerModels, customModelsList] = await Promise.all([
          modelAdapter.getProviderModels(providerId),
          modelAdapter.getCustomModels(providerId)
        ])
        const modelIds = [...providerModels, ...customModelsList].map((model) => model.id)
        const statusMap =
          modelIds.length > 0 ? await modelAdapter.getBatchModelStatus(providerId, modelIds) : {}
  ...
  const refreshStandardModels = async (providerId: string): Promise<void> => {
    try {
      ...
      let models: RENDERER_MODEL_META[] = await modelAdapter.getDbProviderModels(providerId)
      console.log(
      ...
      if (storedModels.length === 0) {
        ...
        const fallbackProviderModels = (await modelAdapter.getProviderModels(providerId)) ?? []
      ...
      if (!models || models.length === 0) {
        try {
          const modelMetas = await modelAdapter.getModelList(providerId)
          if (modelMetas) {
  ...
  const updateModelStatus = async (providerId: string, modelId: string, enabled: boolean) => {
    ...
    try {
      await modelAdapter.updateModelStatus(providerId, modelId, enabled)
  ...
  const addCustomModel = async (
    ...
    try {
      const newModel = await modelAdapter.addCustomModel(providerId, model)
      await modelAdapter.addCustomModelConfig(providerId, newModel)
      await refreshCustomModels(providerId)
      return newModel
    } catch (error) {
  ...
  const removeCustomModel = async (providerId: string, modelId: string) => {
    try {
      await modelAdapter.removeCustomModelConfig(providerId, modelId)
      const success = await modelAdapter.removeCustomModel(providerId, modelId)
      if (success) {
        await refreshCustomModels(providerId)
      }
      return success
  ...
  const updateCustomModel = async (
    ...
    try {
      const success = await modelAdapter.updateCustomModel(providerId, modelId, updates)
      if (success) {
        await refreshCustomModels(providerId)
      }
      return success
  ...
  const enableAllModels = async (providerId: string): Promise<void> => {
    try {
      ...
      for (const model of providerModelsData.models) {
        if (!model.enabled) {
          await modelAdapter.updateModelStatus(providerId, model.id, true)
        }
      }
      await refreshProviderModels(providerId)
    } catch (error) {
  ...
  const disableAllModels = async (providerId: string): Promise<void> => {
    try {
      ...
      for (const model of standardModels) {
        if (model.enabled) {
          await modelAdapter.updateModelStatus(providerId, model.id, false)
        }
      }
  ...
  const setupModelListeners = () => {
    ...
    cleanupModelListeners = modelAdapter.bindModelEvents({
      onModelListChanged: (providerId?: string) => {
        if (providerId) {
