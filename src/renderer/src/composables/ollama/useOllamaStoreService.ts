...
import { useOllamaAdapter } from '@/composables/ollama/useOllamaAdapter'

export const useOllamaStoreService = () => {
  const ollamaAdapter = useOllamaAdapter()
  ...
  const syncOllamaModelsToGlobal = async (providerId: string): Promise<void> => {
    const ollamaProvider = providerStore.providers.find((provider) => provider.id === providerId)
    ...
    const ollamaModelsAsGlobal = await Promise.all(
      local.map(async (model) => {
        ...
        const modelConfig = await ollamaAdapter.getModelConfig(model.name, providerId)
  ...
  const refreshOllamaModels = async (providerId: string): Promise<void> => {
    try {
      const [running, local] = await Promise.all([
        ollamaAdapter.listOllamaRunningModels(providerId),
        ollamaAdapter.listOllamaModels(providerId)
      ])
  ...
  const pullOllamaModel = async (providerId: string, modelName: string) => {
    try {
      ...
      const success = await ollamaAdapter.pullOllamaModels(providerId, modelName)
      if (!success) {
  ...
  const bindEventListeners = () => {
    ...
    const unsubscribe = ollamaAdapter.onPullModelProgress(handleOllamaModelPullEvent)
    const cleanup = () => {
      unsubscribe()
  ...
  const setupOllamaEventListeners = () => {
    if (!cleanupListeners) {
      cleanupListeners = bindEventListeners()
    }
  }
  ...
  const isOllamaModelRunning = (providerId: string, modelName: string): boolean => {
    return getOllamaRunningModels(providerId).some((model) => model.name === modelName)
  }

  const isOllamaModelLocal = (providerId: string, modelName: string): boolean => {
    return getOllamaLocalModels(providerId).some((model) => model.name === modelName)
  }

  const initialize = async () => {
    const ollamaProviders = providerStore.providers.filter(
      (provider) => provider.apiType === 'ollama' && provider.enable
    )
