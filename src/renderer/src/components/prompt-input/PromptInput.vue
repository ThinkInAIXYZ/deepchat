          ...
          <div class="prompt-input-actions flex items-center gap-2 flex-wrap">
            ...
            <ScrollablePopover align="end" content-class="w-80" :enable-scrollable="true">
              ...
              <ChatConfig
                ...
                v-model:enable-search="configEnableSearch"
                v-model:forced-search="configForcedSearch"
                v-model:search-strategy="configSearchStrategy"
...
import { ResourceListEntry, type MODEL_META, type RENDERER_MODEL_META } from '@shared/presenter'
...
watch(
  ...
  ([
    ...
    if (
      newTemp !== currentConfig.temperature ||
      newContext !== currentConfig.contextLength ||
      newMaxTokens !== currentConfig.maxTokens ||
      newSystemPrompt !== currentConfig.systemPrompt ||
      newArtifacts !== currentConfig.artifacts ||
      newThinkingBudget !== currentConfig.thinkingBudget ||
      newEnableSearch !== currentConfig.enableSearch ||
      newForcedSearch !== currentConfig.forcedSearch ||
      newSearchStrategy !== currentConfig.searchStrategy ||
      newReasoningEffort !== currentConfig.reasoningEffort ||
      newVerbosity !== currentConfig.verbosity
    ) {
      chatStore.updateChatConfig({
        temperature: newTemp,
