<template>
  <div class="flex flex-col items-start gap-2">
    <Label class="flex-1 cursor-pointer">
      {{ t('settings.provider.easychatAuth') }}
    </Label>
    ...
    <div v-if="hasToken" class="w-full space-y-2">
      ...
      >
        ...
        <span class="text-sm text-green-700 dark:text-green-300">
          {{ t('settings.provider.easychatConnected') }}
        </span>
      </div>
    ...
    <div v-else class="w-full space-y-2">
      ...
      >
        ...
        <span class="text-sm text-yellow-700 dark:text-yellow-300">
          {{ t('settings.provider.easychatNotConnected') }}
        </span>
      </div>
      <Button
        variant="default"
      ...
      >
        <Icon
          :icon="isLoggingIn ? 'lucide:loader-2' : 'lucide:key'"
      ...
      <div class="text-xs text-muted-foreground">
        {{ t('settings.provider.easychatLoginTip') }}
      </div>
    </div>
...
const startOAuthLogin = async () => {
  ...
  try {
    const success = await oauthPresenter.startEasyChatLogin(props.provider.id)
