...
export type SessionMode = 'agent' | 'acp agent'
...
const currentMode = ref<SessionMode>('agent')
...
export function useSessionMode() {
  ...
  const modes = computed(() => {
    const allModes = [
      { value: 'agent' as SessionMode, label: t('chat.mode.agent'), icon: MODE_ICONS.agent },
      {
        value: 'acp agent' as SessionMode,
  ...
  const loadMode = async () => {
    ...
    try {
      ...
      if (modeUpdateVersion === loadVersion) {
        let savedMode: SessionMode = saved === 'acp agent' ? 'acp agent' : 'agent'
      ...
      console.error('Failed to load session mode, using default:', error)
    } finally {
      hasLoaded = true
    }
  }
