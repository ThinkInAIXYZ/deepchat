<template>
  <div class="w-full h-full overflow-hidden p-2 space-y-3 flex-shrink-0 border-r flex flex-col bg-background">
    ...
    <div class="flex-none flex flex-row gap-2">
      <Button variant="outline" size="sm" class="w-0 flex-1 text-xs justify-start gap-2 h-7" @click="createNewThread">
          ...
          <DynamicScrollerItem :item="item" :active="active" :size-dependencies="[item.data]" :data-index="index">
            ...
            <div v-else-if="item.type === 'thread'" class="px-0 py-1">
              <ThreadItem :thread="item.data" :is-active="item.data.id === chatStore.getActiveThreadId()"
                ...
                @cleanmsgs="showCleanMessagesDialog(item.data)" @addTag="showAddTagDialog(item.data)" />
            </div>
    ...
    <Dialog v-model:open="tagDialog">
      <DialogContent>
        <DialogHeader>
          <DialogTitle>{{ t('dialog.addTag.title') }}</DialogTitle>
          <DialogDescription>{{ t('dialog.addTag.description') }}</DialogDescription>
        </DialogHeader>
        <Input v-if="tagThread" v-model="tagThread.tag" />
        <DialogFooter>
          <Button variant="outline" @click="handleAddTagDialogCancel">{{
            t('dialog.cancel')
          }}</Button>
          <Button variant="default" @click="handleThreadAddTag">{{ t('dialog.confirm') }}</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  </div>
...
const tagThread = ref<CONVERSATION | null>(null)
...
const showAddTagDialog = async (thread: CONVERSATION) => {
  tagDialog.value = true
  tagThread.value = thread
}

const handleAddTagDialogCancel = () => {
  tagDialog.value = false
  tagThread.value = null
}

const handleThreadAddTag = async () => {
  try {
    if (!tagThread.value?.tag) {
      return
    }
    await chatStore.addTagThread(tagThread.value.id, tagThread.value.tag)
  } catch (error) {
    console.error(t('common.error.renameChatFailed'), error)
  }
