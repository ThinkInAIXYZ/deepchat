export interface ProviderCatalogStoragePort {
  readBuiltIn(): string | null
...
export class ProviderDbLoader {
  ...
  constructor(private readonly storage: ProviderCatalogStoragePort) {}
  ...
  private loadFromCache(): ProviderAggregate | null {
    try {
      const raw = this.storage.readCache()
  ...
  private loadFromBuiltIn(): ProviderAggregate | null {
    try {
      const raw = this.storage.readBuiltIn()
      if (raw === null) return null
      const parsed = JSON.parse(raw)
      const sanitized = sanitizeAggregate(parsed)
  ...
  private readMeta(): MetaFile | null {
    try {
      const raw = this.storage.readMeta()
      if (raw === null) return null
  ...
  private writeMeta(meta: MetaFile): void {
    try {
      this.storage.writeMeta(JSON.stringify(meta, null, 2))
    } catch {}
  }

  private writeCacheAtomically(db: ProviderAggregate): void {
    this.storage.writeCache(JSON.stringify(db, null, 2))
  }
  ...
  async refreshIfNeeded(
    ...
    const needFirstFetch = !meta || this.storage.readCache() === null
  ...
  private async fetchAndUpdate(url: string, prevMeta?: MetaFile): Promise<ProviderDbRefreshResult> {
    ...
    try {
      ...
      const res = await this.storage.fetch(url, { headers, signal: controller.signal })
      const now = this.now()
