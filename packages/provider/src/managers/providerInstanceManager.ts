...
import { ProviderBatchUpdate, ProviderChange } from '@deepchat/shared/provider-operations'
import type { LLM_PROVIDER } from '@deepchat/shared/types/provider'
import { BaseLLMProvider } from '../baseProvider.js'
import { OllamaProvider } from '../providers/ollamaProvider.js'
import { VoiceAIProvider } from '../providers/voiceAIProvider.js'
import { AiSdkProvider } from '../providers/aiSdkProvider.js'
import { ApimartProvider } from '../providers/apimartProvider.js'
import { RateLimitManager } from './rateLimitManager.js'
import { StreamState } from '../types.js'
import { resolveAiSdkProviderDefinition } from '../providerRegistry.js'
...
interface ProviderInstanceManagerOptions {
  ...
  host: ProviderHost
  createExtension?: (provider: LLM_PROVIDER) => BaseLLMProvider | undefined
...
export class ProviderInstanceManager {
  ...
  private createProviderInstance(provider: LLM_PROVIDER): BaseLLMProvider | undefined {
    try {
      const extension = this.options.createExtension?.(provider)
      if (extension) return extension

      if (provider.id === 'voiceai') {
        return new VoiceAIProvider(provider, this.options.providerSettings, this.options.host)
      }

      if (provider.id === 'apimart' || provider.apiType === 'apimart') {
        return new ApimartProvider(provider, this.options.providerSettings, this.options.host)
      }

      if (provider.id === 'ollama' || provider.apiType === 'ollama') {
        return new OllamaProvider(provider, this.options.providerSettings, this.options.host)
      }
      ...
      return new AiSdkProvider(provider, this.options.providerSettings, this.options.host)
    } catch (error) {
