import type { ProviderSettingsPort } from '../ports.js'
import { EMBEDDING_TEST_KEY, isNormalized } from '@deepchat/shared/utils/vector'
import {
  ApiEndpointType,
  ModelType,
...
import type { ProviderHost } from '../ports.js'
import {
  runAiSdkCoreStream,
  runAiSdkDimensions,
  runAiSdkEmbeddings,
...
import type { AiSdkProviderKind } from '../aiSdk/providerFactory.js'
import { normalizeAzureBaseUrl, normalizeGeminiBaseUrl } from '../aiSdk/providerFactory.js'
import {
...
export class AiSdkProvider extends BaseLLMProvider {
  ...
  private resolveCapabilityIdentity(
    ...
    return this.host.capabilityResolver.resolveCapabilityIdentity({
  ...
  private resolveNewApiEndpointType(
    ...
    const ownedBy = storedModel?.ownedBy ?? modelConfig.ownedBy
    const capabilityFamilyHint = this.host.capabilityResolver.resolveCapabilityFamilyHint(
      modelId,
      ownedBy
    )
  ...
  private getRuntimeProvider(decision: RouteDecision): LLM_PROVIDER {
    ...
    if (this.host.auth.usesGrokOAuth(base)) {
      const oauthToken = this.host.auth.peekGrokToken()
      if (oauthToken) {
        return {
          ...base,
          oauthToken
  ...
  private buildRuntimeContext(
    ...
    const capabilitySnapshot = this.host.capabilityResolver.buildResolvedCapabilitySnapshot(
      capabilityIdentity,
      {
  ...
  public async requestProviderJson<T>(
    ...
    try {
      ...
      const useGrokOAuth =
        this.host.auth.usesGrokOAuth(runtimeProvider) && this.host.auth.isTrustedGrokEndpoint(url)
      if (useGrokOAuth) {
        await this.host.auth.refreshGrokToken().catch(() => null)
      }
  ...
  private buildEmbeddingRuntimeContext(
    ...
    return {
      ...
      provider: runtimeProvider,
      capabilitySnapshot:
        this.host.capabilityResolver.buildResolvedCapabilitySnapshot(capabilityIdentity),
      supportsOfficialAnthropicReasoning: decision.supportsOfficialAnthropicReasoning,
      providerSettings: this.providerSettings,
      host: this.host,
      defaultHeaders,
  ...
  private mapProviderDbModels(
    ...
    const resolvedId = this.host.capabilities.resolveProviderId(sourceId) || sourceId
    const provider = this.host.catalog.getProvider(resolvedId)
  ...
  private validateCredentials(strategy: AiSdkCredentialStrategy): string | null {
    switch (strategy) {
      case 'api-key':
        if (this.host.auth.usesGrokOAuth(this.provider)) {
          if (this.provider.apiKey || this.host.auth.isGrokAuthenticated()) {
            return null
          }
