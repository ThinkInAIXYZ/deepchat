...
import { isApimartResponsesRoute } from '../capabilityIdentity.js'
...
export class ApimartProvider extends AiSdkProvider {
  ...
  private async downloadVideo(
    ...
    const response = await this.host.fetchRemoteFile(url, {
      signal,
    ...
    try {
      ...
      for (const url of urls) {
        if (kind === 'image') {
          ...
          const data = await this.host.cacheImage(url, { signal, allowPrivateNetwork: false })
