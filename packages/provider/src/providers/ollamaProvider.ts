...
import { normalizeOllamaOpenAIBaseUrl, normalizeOllamaSdkHost } from '../aiSdk/providerFactory.js'
...
export class OllamaProvider extends BaseLLMProvider {
  ...
  protected getAiSdkRuntimeContext(
    ...
    const capabilityIdentity = this.host.capabilityResolver.resolveCapabilityIdentity({
    ...
    return {
      ...
      capabilitySnapshot: this.host.capabilityResolver.buildResolvedCapabilitySnapshot(
        capabilityIdentity,
        {
  ...
  protected async listModelsFromCli(): Promise<OllamaModel[]> {
    return []
  }

  protected isInsecureTlsAllowed(): boolean {
    return false
  }
  ...
  private async attachModelInfo(model: OllamaModel): Promise<OllamaModel> {
    try {
      ...
      const contextLength = this.findModelInfoNumber(
        ...
        (key) =>
          key.endsWith('.context_length.js') &&
          !key.includes('.vision..js') &&
          !key.includes('.audio..js')
      )
      const embeddingLength = this.findModelInfoNumber(
        entries,
        exactPrefixes.map((prefix) => `${prefix}.embedding_length`),
        (key) =>
          key.endsWith('.embedding_length.js') &&
          !key.includes('.vision..js') &&
          !key.includes('.audio..js')
      )
      const visionEmbeddingLength = this.findModelInfoNumber(
        ...
        (key) => key.includes('.vision..js') && key.endsWith('.embedding_length.js')
      )
  ...
  public async pullModel(
    ...
    try {
      const stream = await this.ollama.pull({
        model: modelName,
        insecure: this.isInsecureTlsAllowed(),
        stream: true
      })
