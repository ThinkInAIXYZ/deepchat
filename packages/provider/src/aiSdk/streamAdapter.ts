...
import type { CacheImageOptions } from '../ports.js'
import { parseLegacyFunctionCalls } from './toolProtocol.js'
  ...
  for await (const part of stream) {
    switch (part.type) {
      ...
      case 'file': {
        ...
        if (!options.cacheImage) throw new Error('Media storage is unavailable')
      ...
      case 'finish': {
        ...
        switch (part.finishReason) {
          ...
          case 'other':
            yield createStreamEvent.error(
              `Provider stopped the response for an unspecified reason${part.rawFinishReason ? `: ${part.rawFinishReason}` : './index.js'}`,
