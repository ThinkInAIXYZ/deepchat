...
import { createReasoningMiddleware } from './middlewares/reasoningMiddleware.js'
...
export interface CreateAiSdkProviderContextParams {
  ...
  providerSettings: Pick<ProviderSettingsPort, 'getAzureApiVersion'>
  auth: ProviderHost['auth']
...
export function createAiSdkProviderContext(
  ...
  switch (params.providerKind) {
    case 'openai-codex': {
      const codexBaseUrl = params.auth.normalizeCodexBaseUrl(baseUrl)
      const provider = createOpenAI({
        baseURL: codexBaseUrl,
        apiKey: 'openai-codex-oauth',
        headers: params.defaultHeaders,
        fetch: params.auth.codexFetch(params.defaultHeaders, (url, init) =>
    ...
    case 'openai-compatible': {
      ...
      const openAICompatibleBaseUrl = isOllamaProvider(params.provider)
        ...
        : baseUrl
      const useGrokOAuthFetch = params.auth.usesGrokOAuth(params.provider)
      const resolvedFetch = useGrokOAuthFetch
        ? params.auth.grokFetch(params.provider, params.defaultHeaders, fetch)
        : fetch
