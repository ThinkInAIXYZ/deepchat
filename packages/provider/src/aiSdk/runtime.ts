...
import { mcpToolsToAISDKTools } from './toolMapper.js'
import { mapMessagesToModelMessages } from './messageMapper.js'
import { buildProviderOptions } from './providerOptionsMapper.js'
import {
  type AiSdkProviderKind,
  createAiSdkProviderContext,
  normalizeGeminiBaseUrl
} from './providerFactory.js'
import { adaptAiSdkStream } from './streamAdapter.js'
import {
  learnEmbeddingBatchLimit,
  refreshLearnedEmbeddingBatchLimit,
  resolveEmbeddingBatchLimit
} from './embeddingBatchLimits.js'
import type { PromptCacheIntent } from '../promptCacheStrategy.js'
...
import { createDeepSeekResponsesAdapter } from '@deepchat/shared/provider/deepseekResponsesAdapter'
import { fetchWithProviderHeaders } from '../providerHeaders.js'
...
export async function* runAiSdkCoreStream(
  ...
  if (shouldUseTtsRuntime(context, modelId, normalizedModelConfig)) {
    ...
    const cachedAudio = await context.host.cacheImage(dataUrl, { signal })
    signal?.throwIfAborted()
    yield {
      type: 'image_data',
      image_data: {
  ...
  if (shouldUseImageGenerationRuntime(context, modelId, normalizedModelConfig)) {
    ...
    for (const image of result.images) {
      ...
      const cachedImage = await context.host.cacheImage(dataUrl, { signal: requestSignal })
      requestSignal?.throwIfAborted()
      yield {
        type: 'image_data',
        image_data: {
  ...
  yield* adaptAiSdkStream(result.stream, {
    ...
    cacheImage: context.host.cacheImage.bind(context.host),
