...
import type { ModelCapabilities, CapabilityModelMatch } from './modelCapabilities.js'
import {
...
export function createCapabilityResolver(modelCapabilities: ModelCapabilities) {
  ...
  const resolveCapabilityFamilyHint = (
    modelId: string,
    ownedBy?: string
  ...
  const resolveCapabilityIdentity = (
  ...
  const buildResolvedCapabilitySnapshot = (
  ...
  return { resolveCapabilityFamilyHint, resolveCapabilityIdentity, buildResolvedCapabilitySnapshot }
}
