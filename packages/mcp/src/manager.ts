...
import type {
  McpManagerHost,
  McpManagerSettings as McpSettings,
  McpOAuthPort as McpOAuthManager,
  InMemoryServerFactory
} from './ports.js'
type PrivacySettingsPort = { isEnabled(): boolean }
type SemanticNotificationPublisher = McpManagerHost['notifications']
  ...
  async testNpmRegistrySpeed(useCache: boolean = true): Promise<string> {
    ...
    const results = await Promise.all(
      NPM_REGISTRY_LIST.map(async (registry) => {
        ...
        try {
          success = await this.host.probeRegistry(`${registry}${testPackage}`, controller.signal)
        } catch (error) {
          isTimeout =
