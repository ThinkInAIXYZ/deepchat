...
// The runtime returns only protocol prompt data, not Desktop prompt-library metadata.
type Prompt = {
  id: string
  name: string
  description: string
...
import { createMinimalProcessEnvironment } from './processEnvironment.js'
import {
...
export class McpClient {
  ...
  private mcpOAuthManager?: McpOAuthPort
  ...
  private readonly host: McpClientHost
  ...
  constructor(
    ...
    mcpOAuthManager: McpOAuthPort | undefined,
  ...
  public processCommandWithArgs(
    ...
    return this.host.rewriteCommand(command, args)
  }

  public expandPath(inputPath: string): string {
    return this.host.expandPath(inputPath)
  }

  public get nodeRuntimePath(): string | null {
    try {
      return this.host.runtimeRoot('node')
    } catch {
      return null
    }
  }
  ...
  public get uvRuntimePath(): string | null {
    try {
      return this.host.runtimeRoot('uv')
    } catch {
      return null
    }
  }
  ...
  private addRuntimePathsToEnvironment(
    ...
    this.host.setPathEntriesOnEnv(
      env,
      [this.host.resolvedBinDirs(), inheritedPaths, this.host.getDefaultPaths(homeDir)],
  ...
  private async ensureConnectedForRequest(signal?: AbortSignal): Promise<void> {
    if (!this.isConnected) {
      await this.host.awaitWithAbort(
        this.connect({ phase: 'manual', waitForConnection: true }),
        signal
      )
    }
  ...
  private async performConnect(attempt: number, phase: McpServerStatusPhase): Promise<void> {
    ...
    try {
      ...
      if (this.serverConfig.type === 'inmemory') {
        ...
        command = this.host.expandPath(command)
        args = args.map((arg) => this.host.expandPath(arg))

        const HOME_DIR = this.host.home
        ...
        const processedCommand = this.host.rewriteCommand(command, args)
            ...
            ([key, value]) => {
              if (value !== undefined) {
                ...
                if (['PATH', 'Path', 'path'].includes(key)) {
                  this.host.setPathEntriesOnEnv(
                    env,
                    [stringValue, this.host.getPathEntriesFromEnv(env)],
                    {
                      includeDefaultPaths: false
                    }
                  )
        ...
        this.stdio = this.host.createStdio(
          {
            command,
            args,
            env,
        ...
        this.transport = this.stdio.transport
      ...
      this.client = new Client(this.host.identity, {
        capabilities: {
          sampling: {},
          elicitation: {
  ...
  private async terminateStdio(handle: ManagedMcpStdio): Promise<boolean> {
    try {
  ...
  async forceTerminateStdioProcessTree(reason: string): Promise<boolean> {
    const handle = this.stdio
    if (!handle) return false
    const terminated = await this.terminateStdio(handle)
    if (!terminated) {
      console.warn(
  ...
  private async closeTransport(transport: Transport): Promise<void> {
    const handle = this.stdio?.transport === transport ? this.stdio : undefined
    try {
      await transport.close()
    } finally {
      // Keep the opaque handle until termination is confirmed, including after transport close.
      if (handle) await this.terminateStdio(handle)
    }
  }
  ...
  private async handleSamplingCreateMessage(
    ...
    const decisionWait = this.host.awaitWithAbort(decisionPromise, signal)
  ...
  private async handleElicitationCreate(
    ...
    try {
      const decision = await this.host.awaitWithAbort(decisionPromise, signal)
