...
const packageVersion = JSON.parse(readFileSync(path.resolve(appRoot, 'package.json'), 'utf8')).version

function readPluginManifest(pluginName) {
  const pluginDir = path.resolve(appRoot, 'plugins', pluginName)
...
function discoverOfficialPlugins() {
  const pluginsRoot = path.resolve(appRoot, 'plugins')
...
function stageCuaManagedHelper(pluginDir, targetPlatform, targetArch) {
  ...
  const outRoot = path.resolve(appRoot, 'build', 'managed-helpers')
...
try {
  ...
  const pkgArgs = [path.resolve(appRoot, 'scripts/package-plugin.mjs')]
