...
export class RuntimeHookScope {
  ...
  /**
   * Agent identity for hook facts goes through a read-only registry miss with a persisted-row
   * fallback (SessionIdentityService.getAgentId); it never hydrates a scope and never mutates
   * runtime state, so hook emission stays safe for sessions owned by any backend.
   */
