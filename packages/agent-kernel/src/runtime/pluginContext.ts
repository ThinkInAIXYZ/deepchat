...
export function projectPluginContext(
  assembly: DeepChatPromptAssembly,
  port: PluginContextPort | undefined,
  sessionId: string,
  messageId: string
  ...
  if (contributions.length) {
    const identity = JSON.stringify(
      contributions.map(({ pluginId, digest, invocationId, entryId }) => ({
        pluginId,
        digest,
        invocationId,
        entryId
    ...
    const content = [
      ...
      ...contributions.map(
        (item) =>
          `[Plugin ${item.pluginId}; revision ${item.digest}; invocation ${item.invocationId}; Tape entry ${item.entryId ?? 'unavailable'}]\n${item.content}`
      )
    ].join('\n\n')
    sections.push(
      createPromptAssemblySection({
        kind: 'plugin_context',
        sourceRef: `plugin-context:${createHash('sha256').update(identity).digest('hex')}`,
        content,
        freshness: 'cached'
      })
    )
