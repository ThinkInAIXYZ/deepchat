<p align='center'>
<img src='./packages/desktop/build/icon.png' width="150" height="150" alt="DeepChat AI アシスタントアイコン" />
</p>
...
リポジトリのルートは workspace policy と集約チェックを管理します。Desktop のコマンドは
ルート転送（`pnpm run <script>`）または `pnpm --filter DeepChat run <script>` で実行できます。
後者は `packages/desktop` を明示的に選択します。ここでは移行後の目標レイアウトを記載しているため、
移行前の現在のチェックアウトでは旧パスが残っている場合があります。
