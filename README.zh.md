...
仓库根目录负责 workspace policy 与聚合检查。Desktop 命令可以通过根目录转发（`pnpm run <script>`）
执行，也可以直接使用 `pnpm --filter DeepChat run <script>`；后者会明确选择 `packages/desktop`。
这里记录的是迁移落地后的目标布局，当前迁移前检出可能仍暴露旧路径。
