...
- **卸载：**删除插件及其拥有的本地配置和绑定。不再使用时还需在百智云撤销 Key；卸载不会删除历史会话，
  也不会撤销其他应用持有的凭据。
- **更新：**使用“检查并审阅更新”，检查变化后明确确认。包快照变化后可能需要重新配置凭据，
  即使只更新包内文档，端点和鉴权声明均未改变。已保存的凭据仍在本地存储中，但更新后的插件配置
  不再与该凭据绑定匹配。更新后请检查插件的配置状态；如提示缺少凭据，请在 DeepChat 原生凭据表单中
  重新输入原始 Key。本示例不修改这一行为。修改源码目录不会静默改变已安装的快照。
