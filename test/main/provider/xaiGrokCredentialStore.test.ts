...
import {
  XaiGrokCredentialStore,
  type XaiGrokTokenSet
} from '@/provider/auth/xaiGrok/credentialStore'

const filePath = '/tmp/deepchat-xai-grok/credentials.json'
...
function fileEnvelope(value: XaiGrokTokenSet): string {
  return JSON.stringify({ version: 1, storage: 'file', tokens: value, updatedAt: 1 })
}

describe('XaiGrokCredentialStore', () => {
  ...
  it('returns null without an error when the credential file is missing', () => {
    ...
    const store = new XaiGrokCredentialStore(filePath)
  ...
  it('writes through a temporary file and renames it into place', () => {
    ...
    expect(temporaryPath).toMatch(/^\/tmp\/deepchat-xai-grok\/credentials\.json\.tmp-/)
