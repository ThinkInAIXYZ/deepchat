...
describe('RemoteConversationRunner', () => {
  ...
  it('downloads inbound remote files into the session workspace before sending', async () => {
    ...
    const preparedFile = {
      name: 'note.txt',
      path: path.join(workspace, '.miowork/remote-assets/telegram/hash/message/note.txt'),
      mimeType: 'text/plain',
      content: 'hello file',
      metadata: {
        fileName: 'note.txt',
    ...
    expect(preparedPath).toContain(path.join('.miowork', 'remote-assets', 'telegram'))
  ...
  it('downloads and decrypts inbound encrypted remote media before sending', async () => {
    ...
    const preparedFile = {
      name: 'image-1.png',
      path: path.join(workspace, '.miowork/remote-assets/weixin-ilink/hash/message/image-1.png'),
      mimeType: 'image/png',
      content: '',
      metadata: {
  ...
  it('lists enabled agents only', async () => {
    ...
    expect(agents).toEqual([
      { agentId: 'deepchat', agentName: 'MioWork', agentType: 'deepchat', source: 'builtin' },
      { agentId: 'codex', agentName: 'Codex', agentType: 'acp', source: 'registry' }
    ])
  })
  ...
  it('switches the channel default agent and starts a new session', async () => {
    ...
    const catalog = createCatalog({
      listAgents: vi.fn().mockResolvedValue([
        { agentId: 'deepchat', agentName: 'MioWork', agentType: 'deepchat' },
  ...
  it('rejects an unknown agent id', async () => {
    const catalog = createCatalog({
      listAgents: vi
        .fn()
        .mockResolvedValue([{ agentId: 'deepchat', agentName: 'MioWork', agentType: 'deepchat' }])
    })
    const runner = createRunner(
      {
        catalog,
