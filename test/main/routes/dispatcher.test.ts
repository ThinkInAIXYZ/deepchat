...
function createRuntime() {
  ...
  const projectPresenter = {
    ...
    ensureDefaultWorkspace: vi.fn().mockResolvedValue('C:/Users/test/Documents/MioWork'),
...
describe('dispatchDeepchatRoute', () => {
  ...
  it('ensures the built-in chat workspace before startup bootstrap returns', async () => {
    ...
    vi.mocked(projectPresenter.ensureDefaultWorkspace).mockImplementation(async () => {
      settings.defaultProjectPath = 'C:/Users/test/Documents/MioWork'
      return 'C:/Users/test/Documents/MioWork'
    })
    ...
    expect(result.bootstrap.defaultProjectPath).toBe('C:/Users/test/Documents/MioWork')
    expect(result.bootstrap.defaultChatWorkspacePath).toBe('C:/Users/test/Documents/MioWork')
  })
  ...
  it('returns no memory audit events for missing or non-MioWork agents', async () => {
  ...
  it('rejects directive mutations outside MioWork agents', async () => {
    const { runtime, providerSettings } = createRuntime()
  ...
  it('returns no memory view manifests for missing or non-MioWork agents', async () => {
    const { runtime, providerSettings, tapeInspection } = createRuntime()
  ...
  it('returns an empty memory page for a non-MioWork agent', async () => {
    const { runtime } = createRuntime()
    const pageMemories = vi.fn()
    ;(runtime as any).memoryService = { pageMemories }
  ...
  it('dispatches MioWork agent config routes through AgentSettings', async () => {
    const { runtime, providerSettings } = createRuntime()
    const context = createRendererRouteContext(42, 7)
