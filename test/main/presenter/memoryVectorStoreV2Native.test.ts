...
async function createLegacyV1(paths: MemoryVectorStorePaths, rowCount = 51): Promise<void> {
  ...
  try {
    await connection.run(`LOAD '${escapeSqlPath(VSS_EXTENSION)}';`)
...
async function runCrashWorker(mode: string, paths: MemoryVectorStorePaths): Promise<void> {
  const child = spawn(process.execPath, [CRASH_WORKER, mode, JSON.stringify(paths)], {
    cwd: PROJECT_ROOT,
...
describe('MemoryVectorStore v2 native crash recovery', () => {
  ...
  it.runIf(process.platform === 'win32' && VSS_AVAILABLE)(
    ...
    async () => {
      ...
      const child = spawn(
        process.execPath,
        [CRASH_WORKER, 'hold-quarantined-legacy', JSON.stringify(paths), VSS_EXTENSION],
  ...
  it.runIf(process.platform === 'win32' && VSS_AVAILABLE)(
    ...
    async () => {
      ...
      const child = spawn(
        process.execPath,
        [CRASH_WORKER, 'hold-legacy', JSON.stringify(paths), VSS_EXTENSION],
        { cwd: PROJECT_ROOT, stdio: ['ignore', 'pipe', 'pipe'] }
      )
      try {
        await waitForReady(child)
