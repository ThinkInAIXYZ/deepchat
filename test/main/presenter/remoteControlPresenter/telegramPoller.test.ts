...
describe('TelegramPoller', () => {
  ...
  it('sends markdown answer segments with Telegram HTML parse mode', async () => {
    const client = createClient()
    const bindingStore = createBindingStore()
    client.getUpdates
      .mockResolvedValueOnce([
  ...
  it('retries formatted markdown chunks as plain text when Telegram rejects entities', async () => {
    const client = createClient()
    const bindingStore = createBindingStore()
    client.sendMessage.mockImplementation(async (_target, _text, options) => {
