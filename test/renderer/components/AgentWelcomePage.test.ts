...
describe('AgentWelcomePage', () => {
  it('renders up to nine agents and navigates to MioWork agent settings', async () => {
    vi.resetModules()
    vi.useFakeTimers()
        ...
        t: (key: string) =>
          (
            ({
              ...
              'welcome.agentPage.manageAgents': '管理 MioWork Agent',
              'welcome.agentPage.deepchatType': 'MioWork Agent',
    ...
    // 用 /Agent \d+/ 精确匹配卡片按钮：品牌名 MioWork 自身含 "Agent " 子串，
    // includes('Agent ') 会把「管理 MioWork Agent」按钮误算进来
    ...
    const manageButton = wrapper
      .findAll('button')
      .find((button) => button.text().includes('管理 MioWork Agent'))
