...
describe('ChatStatusBar model and session panels', () => {
  it('shows provider-measured context occupancy for an active MioWork session', async () => {
  ...
  it('does not show MioWork occupancy evidence for an ACP session', async () => {
    const { wrapper } = await setup({
      agentId: 'acp-agent',
      agentType: 'acp',
      hasActiveSession: true,
  ...
  it('shows MioWork occupancy when the agent uses an ACP provider', async () => {
    const { wrapper } = await setup({
  ...
  it('hides collaboration policy for direct ACP but keeps it for MioWork over ACP', async () => {
